...
def main(env=os.environ, coverage_file=".coverage"):
    ...
    api.set_build_values(
        build_url=env["BITBUCKET_CLONE_DIR"], branch=env["BITBUCKET_BRANCH"]
    )

    api.set_dvcs_commit(
        commit_id=env["BITBUCKET_COMMIT"], message="", branch=env["BITBUCKET_BRANCH"]
    )

    api.set_dvcs_user(name_author="", email_author="")
