...
def ensure_array_format(p, shape, *additional_arrays):
    if len(additional_arrays) > 0:
        ...
        assert np.all(
            [a.shape == additional_arrays[0].shape for a in additional_arrays]
    ...
    if shape[0] != "..." and len(p.shape) != len(shape):
        raise ValueError(
            "Expected array of shape %s, got array of shape %s"
            % (str(shape), str(p.shape))
        )
    for i in range(-1, -len(shape) - 1, -1):
        if isinstance(shape[i], str):
            ...
            if shape[i] in number_lookup:
                assert number_lookup[shape[i]] == p.shape[i], (
                    "Expected array of shape %s, got array of shape %s"
                    % (str(shape), str(p.shape))
                )
            number_lookup[shape[i]] = p.shape[i]
        else:
            assert p.shape[i] == shape[i], (
                "Expected array of shape %s, got array of shape %s"
                % (str(shape), str(p.shape))
            )
    return p
